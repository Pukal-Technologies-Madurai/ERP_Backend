...
const insertMobileTemplate = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const checkifNameExist = (await new sql.Request(transaction)
                .input('Report_Name', sql.NVarChar, reportName)
                .query('SELECT COUNT(*) AS name FROM tbl_Mobile_Report_Type WHERE Report_Name = @Report_Name')).recordset[0]?.name;
            ...
            await new sql.Request(transaction)
                ...
                .input('Created_By', sql.Int, createdBy)
                .query(`
                    INSERT INTO tbl_Mobile_Report_Type (Mob_Rpt_Id, Report_Name, Created_By, Created_At, Update_By, Update_At)
                    VALUES (@Mob_Rpt_Id, @Report_Name, @Created_By, GETDATE(), @Created_By, GETDATE())
                `);
            ...
            for (const d of details) {
                const TypeVal = Number(d.Type);
                const TableIdVal = Number(d.Table_Id);
                const FilterLevel = Number(d.FilterLevel || d.Level || (TypeVal >= 4 && TypeVal <= 6 ? 2 : 1));
            ...
            if (GroupFilter && Array.isArray(GroupFilter) && GroupFilter.length > 0) {
                const insertGroupFilterSql = `
                    INSERT INTO tbl_Group_Template (Mob_Rpt_Id, Table_Id, Column_Name, Level_Id)
                    VALUES (@Mob_Rpt_Id, @Table_Id, @Column_Name, @Level_Id)
                `;

                for (const gf of GroupFilter) {
                    if (gf.Column_Name && gf.Table_Id) {
                       
                        let levelId;
                        if (gf.Type) {
                         
                            levelId = Number(gf.Type) - 6; 
                        } else {
                        ...
                        await new sql.Request(transaction)
                            .input('Mob_Rpt_Id', sql.Int, Mob_Rpt_Id)
                            .input('Table_Id', sql.Int, Number(gf.Table_Id))
                            .input('Column_Name', sql.NVarChar, gf.Column_Name)
            ...
            await transaction.commit();
            return success(res, 'Mobile Template Created Successfully', { Mob_Rpt_Id });

        } catch (e) {
            await transaction.rollback();
            console.error('Error in insertMobileTemplate transaction:', e);
            return servError(e, res, 'Error creating mobile template');
        }
    } catch (e) {
        console.error('Error in insertMobileTemplate:', e);
    ...
    try {
        ...
        SELECT
            ...
            COALESCE((
                SELECT 
                    ...
                    tbls.Table_Name, 
                    tbls.Table_Accronym,
                    gf.Level_Id  -- Include Level_Id from tbl_Group_Template
                FROM tbl_Group_Template AS gf
                LEFT JOIN tableMaster AS tbls ON tbls.Table_Id = gf.Table_Id
                WHERE gf.Mob_Rpt_Id = m.Mob_Rpt_Id
                ORDER BY gf.Level_Id  -- Order by Level_Id to maintain slot order
                FOR JSON PATH
            ), '[]') AS groupFiltersList
        FROM mobType AS m
        `;
        ...
        if (reports.length > 0) {
            ...
            if (checkIsNumber(Mob_Rpt_Id)) {
                ...
                const groupFilterArray = (single.groupFiltersList || [])
                    .sort((a, b) => (a.Level_Id || 3) - (b.Level_Id || 3)) 
                    .map(gf => {
            ...
            const allTemplates = parse.map(template => ({
                ...
                Update_At: template.Update_At,
                detailsList: template.detailsList,
                groupFiltersList: template.groupFiltersList.map(gf => ({
                    ...gf,
                    Type: gf.Level_Id ? (gf.Level_Id === 1 ? 7 : gf.Level_Id === 2 ? 8 : 9) : 7,
                    Level_Id: gf.Level_Id 
                })),
                hasGroupFilter: template.groupFiltersList.length > 0
            }));
        ...
        console.error('Error in getMobileTemplates:', e);
        servError(e, res);
    }
};
...
const updateMobileTemplate = async (req, res) => {
    const { Report_Type_Id, reportName, updatedBy, details, GroupFilter } = req.body;
        ...
        try {
            ...
            const nameExists = (await new sql.Request(transaction)
                ...
                .query(`
                    ...
                    WHERE Report_Name = @Report_Name AND Mob_Rpt_Id != @Report_Type_Id
                `)
            ).recordset[0]?.name;
            ...
            for (const d of details) {
                const TypeVal = Number(d.Type);
                const TableIdVal = Number(d.Table_Id);
                const LevelVal = Number(d.Level) || (TypeVal >= 4 && TypeVal <= 6 ? 2 : 1);
            ...
            // Generate and store report query string
            try {
                // Check if Report_Columns column exists and add it if not
                await new sql.Request(transaction).query(`
                    IF NOT EXISTS (
                        SELECT 1 
                        FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            await transaction.rollback();
            console.error('Error in updateMobileTemplate transaction:', err);
            return servError(err, res, 'Error updating mobile template');
        }
    } catch (err) {
        console.error('Error in updateMobileTemplate:', err);
