        ...
        try {
            let query = `
                  ...
                  SELECT DISTINCT
                      ...
                      COALESCE(ecc.Cost_Center_Name, 'Not Assigned') AS Delivery_Person_Name, -- Handle case where no employee is assigned
                  ...
                  LEFT JOIN (
                      -- Aggregate Trip Employees to avoid duplicates
                      SELECT
                          te.Trip_Id,
                          STRING_AGG(ecc.Cost_Center_Name, ', ') AS Cost_Center_Name -- Aggregate employee names
                      FROM tbl_Trip_Employees AS te
                      LEFT JOIN tbl_ERP_Cost_Center AS ecc
                          ON ecc.Cost_Center_Id = te.Involved_Emp_Id
                      GROUP BY te.Trip_Id
                  ) AS ecc ON ecc.Trip_Id = td.Trip_Id
                  WHERE
                      CONVERT(DATE, so.Do_Date) >= CONVERT(DATE, @from)
            ...
            if (Delivery_Person_Id) {
                query += ` AND so.Delivery_Person_Id = @Delivery_Person_Id`;
            }
            if (Route_Id) {
    ...
    try {
        ...
        const result = await request.query(
           ...
           SELECT
               ...
               tm.Trip_Tot_Kms,
               tm.Vehicle_No,
               tm.Branch_Id,  -- Include Branch_Id in the final output
               ...
               COALESCE((  
                   SELECT
                       ...
                       td.Delivery_Do_Date  -- Use the alias
