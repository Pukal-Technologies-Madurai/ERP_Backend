...
const getSaleOrderList = async (req, res) => {
    try {
        ...
        const request = new sql.Request()
            ...
            .input('UserTypeId', checkIsNumber(UserTypeId) ? parseInt(UserTypeId) : null);
        ...
        -- Case 1: Created_by is provided and NOT 0 - Filter by Created_by / UserType
        -- The caller tells us the UserTypeId directly (no lookup needed in
        -- tbl_Users): 0 or 1 means show ALL orders matching the other
        -- filters; any other UserTypeId restricts results to only that
        -- user's own orders.
        IF @creater IS NOT NULL AND @creater != 0
        BEGIN
            IF @UserTypeId IS NOT NULL AND @UserTypeId IN (0, 1)
            BEGIN
                -- WhatsApp-origin user type: show ALL orders matching filters
                INSERT INTO @FilteredOrders (So_Id)
                SELECT so.So_Id
                FROM tbl_Sales_Order_Gen_Info so
                WHERE 
            ...
            ELSE
            BEGIN
                -- Regular user: only their own orders (existing behavior)
                INSERT INTO @FilteredOrders (So_Id)
                SELECT so.So_Id
                FROM tbl_Sales_Order_Gen_Info so
                WHERE 
    ...
    `);
        ...
        const resData = OrderData.map(order => {
            ...
            const convertedto = deliveryList.length > 0 ? 1 : 0;
