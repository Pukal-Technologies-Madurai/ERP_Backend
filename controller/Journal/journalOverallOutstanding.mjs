...
SELECT * FROM (
    ...
    JOIN tbl_Journal_Entries_Info jei ON jgi.JournalAutoId = jei.JournalAutoId AND fj.DrCr = jei.DrCr 
    LEFT JOIN (
        SELECT 
            rbi.bill_id,
            rbi.bill_name,
        ...
        WHERE rgi.status <> 0
        GROUP BY rbi.bill_id, rbi.bill_name, rbi.DR_CR_Acc_Id
    ) rp ON rp.bill_id = jgi.JournalId AND rp.bill_name = jgi.JournalVoucherNo AND rp.DR_CR_Acc_Id = jei.Acc_Id AND jei.DrCr = 'Dr'
    LEFT JOIN (
        SELECT 
            pb.pay_bill_id,
            pb.bill_name,
        ...
        WHERE pgi.status <> 0
        GROUP BY pb.pay_bill_id, pb.bill_name, pb.DR_CR_Acc_Id
    ) pb ON pb.pay_bill_id = jgi.JournalId AND pb.bill_name = jgi.JournalVoucherNo AND pb.DR_CR_Acc_Id = jei.Acc_Id AND jei.DrCr = 'Cr'
    LEFT JOIN (
        SELECT 
            jr.RefId,
            jr.RefNo,
